...
def drive_straight_cm(cm,speed=DEFAULT_SPEED,kp=2.0):
    target_deg=(abs(cm)/WHEEL_CIRCUMFERENCE_CM)*360; left_motor.set_degrees_counted(0); reset_yaw()
...
def drive_until_wall(speed=DEFAULT_SPEED,min_cm=WALL_APPROACH_CM,timeout_s=4.0):
    ...
    from .hardware import safe_get_distance_cm
    while time()-t<timeout_s:
        if safe_get_distance_cm()<=min_cm: drive_pair.stop(); return True
